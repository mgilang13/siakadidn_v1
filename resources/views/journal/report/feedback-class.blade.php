...
@section('content')
<div class="content">
    <h1 class="text-center text-md-left h1-responsive" name="top">Laporan Feedback per Tingkat</h1>
    <h3>Laporan Feedback Kelas</h3>
    <div class="card shadow">
        <div class="card-title mt-2">
            <h2>Filter</h2>
        </div>
        <div class="card-body">
            <form action="{{ route('journal.report.feedback-class') }}" class="form-inline">
    ...
    <div class="d-flex flex-wrap row-cols-1 row-cols-md-3 mt-4" ">
        @forelse($feedback_class as $fc)
            <div class="col mb-4">
                <div class="card shadow pb-2">
                    <canvas id="canvas{{ $fc->id_class }}"></canvas>
                </div>
            </div>
        @empty
        <div>
...
<script>
        $(document).ready(function() {
            let feedback_class = {!! json_encode($feedback_class) !!}

            feedback_class.map(result => {
                console.log(result);
                ...
                let data = [parseFloat(result.persen_sudahpaham).toFixed(2), parseFloat(result.persen_belumpaham).toFixed(2), parseFloat(result.persen_belumngisi).toFixed(2)];
                let labels = ["Sudah Paham", "Belum Paham", "Belum Mengisi"];
                ...
                let chart = new Chart(ctx, {
                    ...
                    options:{
                        ...
                        title: {
                            display: true,
                            text: "Laporan feedback Kelas: "+result.classname
                        },
                        legend:{
                            display:true,
                            position:'left'
